...
render(mainBlock, new SortComponent(filters).getElement(), RenderPosition.BEFOREEND);
...
if (films.length === 0) {
  render(boardBlock.getElement(), new EmptyListComponent().getElement(), RenderPosition.BEFOREEND);
} else {
  const renderCard = (container, film) => {
    ...
    const onEscKeyDown = (evt) => {
      ...
      if (isEscKey) {
        bodyBlock.removeChild(cardPopupComponent.getElement());
